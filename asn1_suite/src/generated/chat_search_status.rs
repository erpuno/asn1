...
#[derive(Clone, Debug)]
pub enum CHATSearchStatus {
    Ok = 0,
}

impl DERParseable for CHATSearchStatus {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        ...
        match val {
            0 => Ok(CHATSearchStatus::Ok),
            _ => Err(ASN1Error::new(rust_asn1::errors::ErrorCode::InvalidASN1Object, format!("Unknown value for CHATSearchStatus: {}", val), file!().to_string(), line!()))
        }
    }
}

impl DERSerializable for CHATSearchStatus {
    fn serialize(&self, serializer: &mut Serializer) -> Result<(), ASN1Error> {
        let val = match self {
            CHATSearchStatus::Ok => 0,
        };
