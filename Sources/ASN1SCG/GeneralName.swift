...
@usableFromInline indirect enum GeneralName: DERParseable, DERSerializable, Hashable, Sendable {
    ...
    @inlinable init(derEncoded rootNode: ASN1Node) throws {
        switch rootNode.identifier {
            case ASN1Identifier(tagWithNumber: 0, tagClass: .contextSpecific):
                self = .otherName(try GeneralName(derEncoded: rootNode))
            case ASN1Identifier(tagWithNumber: 1, tagClass: .contextSpecific):
                self = .rfc822Name(try ASN1IA5String(derEncoded: rootNode))
            case ASN1Identifier(tagWithNumber: 2, tagClass: .contextSpecific):
                self = .dNSName(try ASN1IA5String(derEncoded: rootNode))
            case ASN1Identifier(tagWithNumber: 3, tagClass: .contextSpecific):
                self = .x400Address(try ORAddress(derEncoded: rootNode))
            case ASN1Identifier(tagWithNumber: 4, tagClass: .contextSpecific):
                self = .directoryName(try Name(derEncoded: rootNode))
            case ASN1Identifier(tagWithNumber: 5, tagClass: .contextSpecific):
                self = .ediPartyName(try EDIPartyName(derEncoded: rootNode))
            ...
            case ASN1Identifier(tagWithNumber: 7, tagClass: .contextSpecific):
                self = .iPAddress(try ASN1OctetString(derEncoded: rootNode))
            case ASN1Identifier(tagWithNumber: 8, tagClass: .contextSpecific):
                self = .registeredID(try ASN1ObjectIdentifier(derEncoded: rootNode))
    ...
    @inlinable func serialize(into coder: inout DER.Serializer) throws {
        switch self {
            ...
            case .x400Address(let x400Address):
                try coder.appendConstructedNode(
                identifier: ASN1Identifier(tagWithNumber: 3, tagClass: .contextSpecific),
                { coder in try coder.serialize(x400Address) })
            ...
            case .ediPartyName(let ediPartyName):
                try coder.appendConstructedNode(
                identifier: ASN1Identifier(tagWithNumber: 5, tagClass: .contextSpecific),
                { coder in try coder.serialize(ediPartyName) })
