...
@usableFromInline struct Profile: DERImplicitlyTaggable, Hashable, Sendable {
    ...
    @inlinable init(derEncoded root: ASN1Node,
        ...
        self = try DER.sequence(root, identifier: identifier) { nodes in
            ...
            let chats = try DER.sequence(of: Contact.self, identifier: .sequence, nodes: &nodes)
            let contacts = try DER.sequence(of: Contact.self, identifier: .sequence, nodes: &nodes)
            let keys = try DER.sequence(of: ASN1OctetString.self, identifier: .sequence, nodes: &nodes)
            let servers = try DER.sequence(of: Server.self, identifier: .sequence, nodes: &nodes)
            ...
            let status = try ArraySlice<UInt8>(derEncoded: &nodes)
            let roster = try Roster(derEncoded: &nodes)
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            ...
            try coder.serialize(self.roster)
        }
    }
}
