...
@usableFromInline struct Message: DERImplicitlyTaggable, Hashable, Sendable {
    ...
    @inlinable init(derEncoded root: ASN1Node,
        ...
        self = try DER.sequence(root, identifier: identifier) { nodes in
            ...
            let feed_id = try Message_feed_id_Choice(derEncoded: &nodes)
            ...
            let files = try DER.sequence(of: FileDesc.self, identifier: .sequence, nodes: &nodes)
            let type = try MessageType(derEncoded: &nodes)
            ...
            let status = try MessageStatus(derEncoded: &nodes)
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            ...
            try coder.serialize(self.from)
            try coder.serialize(self.to)
            try coder.serialize(self.created)
            try coder.serializeSequenceOf(files)
            try coder.serialize(self.type)
            try coder.serialize(self.link)
            try coder.serialize(self.seenby)
            try coder.serialize(self.repliedby)
