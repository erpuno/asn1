// Generated by ASN1.ERP.UNO Compiler, Copyright © 2023 Namdak Tonpa.
public struct AttCertVersion : Hashable, Sendable, Comparable {
    @usableFromInline  var rawValue: Int
    @inlinable public static func < (lhs: AttCertVersion, rhs: AttCertVersion) -> Bool { lhs.rawValue < rhs.rawValue }
    @inlinable init(rawValue: Int) { self.rawValue = rawValue }
    public static let v2 = Self(rawValue: 1)
}
