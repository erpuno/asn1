...
@usableFromInline struct CHATMessage: DERImplicitlyTaggable, Hashable, Sendable {
    ...
    @inlinable init(derEncoded root: ASN1Node,
        ...
        self = try DER.sequence(root, identifier: identifier) { nodes in
            let no = try ArraySlice<UInt8>(derEncoded: &nodes)
            let headers = try DER.sequence(of: ASN1OctetString.self, identifier: .sequence, nodes: &nodes)
            let body = try CHATProtocol(derEncoded: &nodes)
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            try coder.serialize(self.no)
            try coder.serializeSequenceOf(headers)
            try coder.serialize(self.body)
        }
    }
}
