...
@usableFromInline struct AuthenticationFramework_CertificatePair: DERImplicitlyTaggable, Hashable, Sendable {
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            if let issuedByThisCA = self.issuedByThisCA { if let issuedByThisCA = self.issuedByThisCA { try coder.serialize(explicitlyTaggedWithTagNumber: 0, tagClass: .contextSpecific) { codec in try codec.serialize(issuedByThisCA) } } }
            if let issuedToThisCA = self.issuedToThisCA { if let issuedToThisCA = self.issuedToThisCA { try coder.serialize(explicitlyTaggedWithTagNumber: 1, tagClass: .contextSpecific) { codec in try codec.serialize(issuedToThisCA) } } }
        }
    }
}
