...
@usableFromInline struct SelectedAttributeTypes_TimeSpecification_time_Choice_absolute_Sequence: DERImplicitlyTaggable, Hashable, Sendable {
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            if let startTime = self.startTime { if let startTime = self.startTime { try coder.serialize(explicitlyTaggedWithTagNumber: 0, tagClass: .contextSpecific) { codec in try codec.serialize(startTime) } } }
