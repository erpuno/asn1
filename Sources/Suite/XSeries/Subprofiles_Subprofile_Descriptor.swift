...
@usableFromInline struct Subprofiles_Subprofile_Descriptor: DERImplicitlyTaggable, Sendable {
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            ...
            if let subprofile_reference = self.subprofile_reference { try coder.serializeOptionalImplicitlyTagged(subprofile_reference, withIdentifier: ASN1Identifier(tagWithNumber: 1, tagClass: .contextSpecific)) }
            if let subprofile_precedence = self.subprofile_precedence { try coder.serializeOptionalImplicitlyTagged(subprofile_precedence, withIdentifier: ASN1Identifier(tagWithNumber: 2, tagClass: .contextSpecific)) }
            ...
            if let content_architecture_classes = self.content_architecture_classes { try coder.serializeSetOf(content_architecture_classes, identifier: ASN1Identifier(tagWithNumber: 4, tagClass: .contextSpecific)) }
            if let document_fragment_management_attributes = self.document_fragment_management_attributes { try coder.serializeOptionalImplicitlyTagged(document_fragment_management_attributes, withIdentifier: ASN1Identifier(tagWithNumber: 5, tagClass: .contextSpecific)) }
