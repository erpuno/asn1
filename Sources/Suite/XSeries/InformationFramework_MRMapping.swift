...
@usableFromInline struct InformationFramework_MRMapping: DERImplicitlyTaggable, Hashable, Sendable {
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            if let mapping = self.mapping { if let mapping = self.mapping { try coder.serialize(explicitlyTaggedWithTagNumber: 0, tagClass: .contextSpecific) { codec in try codec.serializeSequenceOf(mapping) } } }
            if let substitution = self.substitution { if let substitution = self.substitution { try coder.serialize(explicitlyTaggedWithTagNumber: 1, tagClass: .contextSpecific) { codec in try codec.serializeSequenceOf(substitution) } } }
        }
    }
}
