...
@usableFromInline struct AuthenticationFramework_ACPathData: DERImplicitlyTaggable, Hashable, Sendable {
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            if let certificate = self.certificate { if let certificate = self.certificate { try coder.serialize(explicitlyTaggedWithTagNumber: 0, tagClass: .contextSpecific) { codec in try codec.serialize(certificate) } } }
            if let attributeCertificate = self.attributeCertificate { if let attributeCertificate = self.attributeCertificate { try coder.serialize(explicitlyTaggedWithTagNumber: 1, tagClass: .contextSpecific) { codec in try codec.serialize(attributeCertificate) } } }
        }
    }
}
