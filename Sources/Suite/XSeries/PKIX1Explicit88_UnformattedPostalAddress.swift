...
@usableFromInline struct PKIX1Explicit88_UnformattedPostalAddress: DERImplicitlyTaggable, Hashable, Sendable {
...
if let next = peek_teletex_string.next(), next.identifier == ASN1TeletexString.defaultIdentifier {
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            if let printable_address = self.printable_address { if let printable_address = self.printable_address { try coder.serializeSequenceOf(printable_address) } }
