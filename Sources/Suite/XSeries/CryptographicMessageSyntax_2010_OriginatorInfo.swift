...
@usableFromInline struct CryptographicMessageSyntax_2010_OriginatorInfo: DERImplicitlyTaggable, Hashable, Sendable {
    ...
    @inlinable func serialize(into coder: inout DER.Serializer,
        ...
        try coder.appendConstructedNode(identifier: identifier) { coder in
            if let certs = self.certs { if let certs = self.certs { try coder.serializeOptionalImplicitlyTagged(certs, withIdentifier: ASN1Identifier(tagWithNumber: 0, tagClass: .contextSpecific)) } }
