...
@usableFromInline indirect enum Filter: DERImplicitlyTaggable, DERParseable, DERSerializable, Hashable, Sendable {
    ...
    @inlinable init(derEncoded rootNode: ASN1Node, withIdentifier: ASN1Identifier) throws {
        switch rootNode.identifier {
            case ASN1Identifier(tagWithNumber: 0, tagClass: .contextSpecific):
                self = .and(try DER.set(of: Filter.self, identifier: .set, rootNode: rootNode))
            case ASN1Identifier(tagWithNumber: 1, tagClass: .contextSpecific):
                self = .or(try DER.set(of: Filter.self, identifier: .set, rootNode: rootNode))
            case ASN1Identifier(tagWithNumber: 2, tagClass: .contextSpecific):
                self = .not(try Filter(derEncoded: rootNode, withIdentifier: rootNode.identifier))
            case ASN1Identifier(tagWithNumber: 3, tagClass: .contextSpecific):
                self = .equalityMatch(try AttributeValueAssertion(derEncoded: rootNode, withIdentifier: rootNode.identifier))
            case ASN1Identifier(tagWithNumber: 4, tagClass: .contextSpecific):
                self = .substrings(try SubstringFilter(derEncoded: rootNode, withIdentifier: rootNode.identifier))
            case ASN1Identifier(tagWithNumber: 5, tagClass: .contextSpecific):
                self = .greaterOrEqual(try AttributeValueAssertion(derEncoded: rootNode, withIdentifier: rootNode.identifier))
            case ASN1Identifier(tagWithNumber: 6, tagClass: .contextSpecific):
                self = .lessOrEqual(try AttributeValueAssertion(derEncoded: rootNode, withIdentifier: rootNode.identifier))
            ...
            case ASN1Identifier(tagWithNumber: 8, tagClass: .contextSpecific):
                self = .approxMatch(try AttributeValueAssertion(derEncoded: rootNode, withIdentifier: rootNode.identifier))
            case ASN1Identifier(tagWithNumber: 9, tagClass: .contextSpecific):
                self = .extensibleMatch(try MatchingRuleAssertion(derEncoded: rootNode, withIdentifier: rootNode.identifier))
    ...
    @inlinable func serialize(into coder: inout DER.Serializer, withIdentifier: ASN1Identifier) throws {
        switch self {
            case .and(let and):
                ...
                { coder in try coder.serializeSetOf(and) })
            case .or(let or):
                try coder.appendConstructedNode(
                identifier: ASN1Identifier(tagWithNumber: 1, tagClass: .contextSpecific),
                { coder in try coder.serializeSetOf(or) })
            case .not(let not):
                try coder.appendConstructedNode(
