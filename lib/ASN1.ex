defmodule ASN1 do
  ...
  def enumeration(name, cases, modname, save), do: emitter().enumeration(name, cases, modname, save)
  def integerEnum(name, cases, modname, save), do: emitter().integerEnum(name, cases, modname, save)
  def substituteType(type), do: emitter().substituteType(type)
  def tagClass(tag), do: emitter().tagClass(tag)
  def value(name, type, val, modname, saveFlag), do: emitter().value(name, type, val, modname, saveFlag)
  ...
  defp normalizeClassName(name) do
      ...
      if String.upcase(normalized) == normalized and String.length(normalized) > 1 and not String.contains?(normalized, "_") do
  ...
  def compileType(pos, name, typeDefinition, modname, save \\ true) do
      res = case typeDefinition do
          {:type, _, {:"INTEGER", cases}, _, _, :no} ->  setEnv(name, builtinType(:'INTEGER')) ; integerEnum(name, cases, modname, save)
          ...
          {:type, _, :'BIT STRING', _, [], :no} -> typealias(name, builtinType(:'BIT STRING'), modname, save)
          {:type, _, {:'BIT STRING', _}, _, [], :no} -> typealias(name, builtinType(:'BIT STRING'), modname, save)
          {:type, _, :'INTEGER', _set, [], :no} -> typealias(name, builtinType(:'INTEGER'), modname, save)
          {:type, _, :'NULL', _set, [], :no} -> typealias(name, builtinType(:'NULL'), modname, save)
          {:type, _, :'ANY', _set, [], :no} -> typealias(name, builtinType(:'ANY'), modname, save)
          {:type, _, :'OBJECT IDENTIFIER', _set, _constraints, :no} -> typealias(name, builtinType(:'OBJECT IDENTIFIER'), modname, save)
          {:type, _, :'External', _set, [], :no} -> setEnv(name, builtinType(:'External'))
          {:type, _, :'PrintableString', _set, [], :no} -> typealias(name, builtinType(:'PrintableString'), modname, save)
          {:type, _, :'PrintableString', _set, _constraints, :no} -> typealias(name, builtinType(:'PrintableString'), modname, save)
          {:type, _, :'NumericString', _set, [], :no} -> typealias(name, builtinType(:'NumericString'), modname, save)
          {:type, _, :'NumericString', _set, _constraints, :no} -> typealias(name, builtinType(:'NumericString'), modname, save)
          {:type, _, :'IA5String', _set, [], :no} -> typealias(name, builtinType(:'IA5String'), modname, save)
          {:type, _, :'TeletexString', _set, [], :no} -> typealias(name, builtinType(:'TeletexString'), modname, save)
          {:type, _, :'UniversalString', _set, [], :no} -> typealias(name, builtinType(:'UniversalString'), modname, save)
          {:type, _, :'UTF8String', _set, [], :no} -> typealias(name, builtinType(:'UTF8String'), modname, save)
          {:type, _, :'VisibleString', _set, [], :no} -> typealias(name, builtinType(:'VisibleString'), modname, save)
          {:type, _, :'BMPString', _set, [], :no} -> typealias(name, builtinType(:'BMPString'), modname, save)
          ...
          {:'Object', _, _} -> :ignore
          {:'Object', _, _, _} -> :ignore
          {:'Object', _, _, _, _} -> :ignore
          {:'ObjectSet', _, _} -> :ignore
          {:'ObjectSet', _, _, _} -> :ignore
  ...
  def compileValue(_pos, name, {:type, [], :'OBJECT IDENTIFIER', [], [], :no} = type, val, mod), do: emitter().value(name, type, val, mod, true)
  def compileValue(_pos, name, {:type, _, {:Externaltypereference, _, _, ref}, _, _, _} = type, val, mod) do
      resolved = lookup(bin(ref))
      if resolved == builtinType(:ASN1ObjectIdentifier) or resolved == "OBJECT IDENTIFIER" or getEnv({:is_oid, resolved}, false) do
  ...
  def save(true, _, name, res) do
      ...
      fileName = dir <> norm <> emitter().fileExtension()
      verbose = getEnv(:verbose, false)
      case :lists.member(norm,exceptions()) do
           true ->  print "skipping: ~ts~ts~n", [norm, emitter().fileExtension()] ; setEnv(:verbose, verbose)
